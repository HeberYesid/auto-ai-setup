import type { ApprovalDecisions, ChangePlan, ComponentDefinition, ComponentSelectionView, CompatibilityDecision, ConfirmedStack, RedactedEvent, RunMode, StackConflict, UserInteraction } from "../domain/index.js";
...
const listInput = (value: string): readonly string[] => [...new Set(value.split(/[\s,]+/u).map((entry) => entry.trim()).filter(Boolean))];
...
export class InteractiveUserInteraction implements UserInteraction {
  public constructor(private readonly terminal: CliTerminal, private readonly verbose = false) {}
  ...
  async resolveStackSelection(conflicts: readonly StackConflict[]): Promise<Readonly<Partial<Record<StackConflict["category"], string>>>> {
    ...
    for (const conflict of conflicts) {
      ...
      const candidate = Number.isInteger(index) ? conflict.candidates[index] : conflict.candidates.find((entry) => entry.id === selected.trim());
  ...
  async resolveStack(conflicts: readonly StackConflict[]): Promise<ConfirmedStack> {
    ...
    const items = conflicts.flatMap((conflict) => conflict.candidates.filter((candidate) => selections[conflict.category] === candidate.id));
  ...
  async selectComponents(view: ComponentSelectionView): Promise<readonly import("../domain/index.js").ComponentId[]> {
    ...
    for (const component of view.components) this.terminal.write(`  ${component.definition.id} — ${component.definition.name}${component.compatibility.compatible ? "" : ` [incompatible: ${component.compatibility.unsatisfied.join("; ")}]`}`);
  ...
  async reviewPlan(plan: ChangePlan): Promise<ApprovalDecisions> {
    ...
    for (const change of plan.fileChanges.filter((entry) => entry.conflict !== "none" && (entry.action === "create" || entry.action === "modify"))) {
      conflicts[change.id] = (await this.ask(`Conflicto en ${canonical(plan.root, change.destination)}: ¿reemplazar?`)) ? "replace" : "preserve";
    }
    const needsGlobal = plan.fileChanges.some((change) => change.conflict === "none" && (change.action === "create" || change.action === "modify")) || plan.externalOperations.length > 0;
    const globalApproved = needsGlobal && await this.ask("¿Aprobar el plan completo?");
    const networkOperations: string[] = [];
    for (const operation of plan.externalOperations) if (await this.ask(`¿Autorizar operación de red ${operation.id}?`)) networkOperations.push(operation.id);
    const hash = asSha256(plan.planHash);
    if (!hash.ok) throw new Error("El hash del plan no es válido");
    return { planHash: hash.value, globalApproved, conflicts, incompatibleComponents: [...new Set(plan.fileChanges.flatMap((change) => change.incompatibleOverride === undefined ? [] : [change.componentId]))], networkOperations: networkOperations as import("../domain/index.js").OperationId[] };
  }
  ...
  private renderPlan(plan: ChangePlan): void {
    ...
    for (const change of plan.fileChanges) {
      this.terminal.write(`- ${change.action.toUpperCase()} ${canonical(plan.root, change.destination)} | componente=${change.componentId} | motivo=${change.reason} | conflicto=${change.conflict}`);
      if (change.preview.kind === "text") this.terminal.write(`  preview: ${change.preview.content}`);
      else for (const field of change.preview.changes) this.terminal.write(`  ${field.action} ${field.path}: ${JSON.stringify(field.before)} -> ${JSON.stringify(field.after)}`);
    }
    for (const operation of plan.externalOperations) this.terminal.write(`- EXTERNAL ${operation.id} | comando=${operation.command.join(" ")} | origen=${operation.origin} | destino=${canonical(plan.root, operation.destination)} | propósito=${operation.purpose} | red=${operation.usesNetwork ? "sí" : "no"}`);

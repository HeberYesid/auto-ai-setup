...
const conflictOrder: Record<FileChange["conflict"], number> = { none: 0, "content-differs": 1, "invalid-managed-markers": 2, "ownership-unknown": 3 };
...
const invalid = (message: string, path?: string): Result<ChangePlan, PlanningError> => err({
  code: "INVALID_PLAN",
  message,
  ...(path === undefined ? {} : { path }),
  recoverability: "none",
...
const destinationError = (destination: string): Result<ChangePlan, PlanningError> => err({
...
const sortFieldChanges = (changes: readonly FieldChange[]): readonly FieldChange[] => [...changes]
  .map((change) => ({ ...change }))
...
const normalizePreview = (preview: FileChange["preview"], redactor: SecretRedactor): FileChange["preview"] => {
  if (preview.kind === "text") return { kind: "text", content: text(redactor.redact(preview.content)), truncated: preview.truncated } satisfies RedactedPreview;
  return {
    kind: "fields",
...
export class DeterministicChangePlanner {
  ...
  public constructor(options: PlannerOptions = {}) { this.redactor = options.redactor ?? new SecretRedactor(); }

  public async build(input: PlanningInput): Promise<Result<ChangePlan, PlanningError>> {
    if (input.root.length === 0 || input.root.includes("\0") || input.runId.length === 0 || input.now.length === 0) return invalid("Plan identity and root are required");
    ...
    for (const original of input.fileChanges) {
      ...
      if (destinations.has(checked.value)) return invalid("A plan cannot contain more than one file action for a destination", checked.value);
      destinations.add(checked.value);
      fileChanges.push(normalizeFileChange({ ...original, destination: checked.value as unknown as FileChange["destination"] }, this.redactor));
    }
    const operationIds = new Set<string>();
    const externalOperations: ExternalOperation[] = [];
    for (const original of input.externalOperations) {
      if (operationIds.has(original.id) || original.id.length === 0 || original.command.length === 0 || original.command.some((argument) => argument.includes("\0"))) return invalid("External operation identities and commands must be unique and safe", original.id);
      ...
      externalOperations.push(normalizeExternal({ ...original, destination: destination.value as unknown as ExternalOperation["destination"] }, this.redactor));
    }
    fileChanges.sort((left, right) => left.destination.localeCompare(right.destination) || actionOrder[left.action] - actionOrder[right.action] || conflictOrder[left.conflict] - conflictOrder[right.conflict] || left.id.localeCompare(right.id));
    externalOperations.sort((left, right) => left.destination.localeCompare(right.destination) || left.kind.localeCompare(right.kind) || left.id.localeCompare(right.id));

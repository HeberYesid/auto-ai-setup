...
const packageFiles: readonly FilePattern[] = [
  { pattern: "package.json", format: "json" },
  { pattern: "package-lock.json", format: "lockfile" },
  { pattern: "pnpm-lock.yaml", format: "yaml" },
  { pattern: "yarn.lock", format: "lockfile" },
  { pattern: "bun.lockb", format: "lockfile" },
];
const pythonFiles: readonly FilePattern[] = [
  { pattern: "pyproject.toml", format: "toml" },
  { pattern: "requirements.txt", format: "lockfile" },
  { pattern: "poetry.lock", format: "lockfile" },
  { pattern: "uv.lock", format: "lockfile" },
];
const rubyFiles: readonly FilePattern[] = [
  { pattern: "Gemfile", format: "lockfile" },
  { pattern: "Gemfile.lock", format: "lockfile" },
];
const phpFiles: readonly FilePattern[] = [
  { pattern: "composer.json", format: "json" },
  { pattern: "composer.lock", format: "lockfile" },
];
...
const hasTextDependency = (source: string, names: readonly string[]): string | undefined =>
  names.find((name) => new RegExp(`(?:^|[\\s"'])${escapeRegExp(name)}(?:$|[\\s"'=:/])`, "imu").test(source));
const escapeRegExp = (value: string): string => value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");

const valid = (file: ParsedEvidence): boolean => file.validSyntax && file.validSchema;
const packageManagerDetector = (
  id: string,
  files: readonly FilePattern[],
  manager: string,
...
const languageDetector = (
  ...
  detect: (file) => {
    ...
    return [
      claim("language", id, displayName, file.format === "source-extension" ? "derived" : "explicit", file, file.path, `language.${id}`),
    ];
  },
});

const packageDependencyDetector = (
  id: string,
  displayName: string,
  names: readonly string[],
  category: StackCategory,
  files: readonly FilePattern[] = packageFiles,
): StackDetector => ({
  id,
  acceptedFiles: files,
  detect: (file) => {
...
const sourcePatterns = (extensions: readonly string[]): readonly FilePattern[] =>
  extensions.map((extension) => ({ pattern: `*${extension}`, format: "source-extension" as const }));
...
const detectors: readonly StackDetector[] = [
  languageDetector(
    ...
    "JavaScript",
    [".js", ".jsx", ".mjs", ".cjs"],
    [...sourcePatterns([".js", ".jsx", ".mjs", ".cjs"]), ...packageFiles, { pattern: "tsconfig.json", format: "json" }],
  ),
  languageDetector(
    "typescript",
    "TypeScript",
    [".ts", ".tsx", ".mts", ".cts"],
    [...sourcePatterns([".ts", ".tsx", ".mts", ".cts"]), ...packageFiles, { pattern: "tsconfig.json", format: "json" }],
  ),
...
const matches = (path: string, pattern: FilePattern): boolean => {
  ...
  if (normalizedPattern.startsWith("**/")) {
    ...
    return (
      normalizedPath.includes(suffix.slice(0, suffix.lastIndexOf("/"))) &&
      normalizedPath.endsWith(suffix.slice(suffix.lastIndexOf("/") + 1))
    );
  }
...
const configDetector = (id: string, displayName: string, names: readonly string[], category: StackCategory): StackDetector => ({
  id,
  acceptedFiles: names.map((name) => ({
    pattern: name,
    format: name.endsWith(".json") ? "json" : name.endsWith(".toml") ? "toml" : "source-extension",
  })),
  detect: (file) => {
    if (!valid(file)) return [];
    const name = file.path.split("/").pop()?.toLowerCase() ?? "";
...
const configDetectors: readonly StackDetector[] = [
  ...
  configDetector(
    ...
    "Prettier",
    [".prettierrc", ".prettierrc.json", "prettier.config.js", "prettier.config.cjs"],
    "tool",
  ),

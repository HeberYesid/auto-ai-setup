...
const digestValue = (value: unknown, key = ""): unknown => {
  ...
  if (value !== null && typeof value === "object") return Object.fromEntries(Object.entries(value as Record<string, unknown>).sort(([left], [right]) => left.localeCompare(right)).map(([childKey, child]) => [childKey, digestValue(child, childKey)]));
  return value;
};
const defaultDigest = (component: ComponentDefinition): Sha256 => createHash("sha256").update(JSON.stringify(digestValue(component)), "utf8").digest("hex") as Sha256;
...
export class ManagedStateOwnership {
  ...
  public constructor(private readonly store: SkillOwnershipStore, options: ManagedStateOwnershipOptions = {}) {
    this.digest = options.digest ?? ((component) => defaultDigest(component));
  }

  public load(): Promise<Result<ManagedState | undefined>> { return this.store.load(); }
...
export const createManagedStateOwnership = (store: SkillOwnershipStore, options?: ManagedStateOwnershipOptions): ManagedStateOwnership => new ManagedStateOwnership(store, options);

...
const failure = (message: string, code: "UNEXPECTED_ERROR" | "VERIFY_FAILED" | "WRITE_FAILED" = "UNEXPECTED_ERROR"): AppError => ({ code, message, recoverability: "rollback" });
const journalPath = (runId: RunId): SafeProjectPath => `${TRANSACTION_ROOT}/${runId}/journal.json` as SafeProjectPath;
const backupPath = (runId: RunId, index: number): SafeProjectPath => `${TRANSACTION_ROOT}/${runId}/backups/${String(index).padStart(6, "0")}.bak` as SafeProjectPath;

const unsignedPlan = (plan: ApprovedPlan): Omit<ApprovedPlan, "planHash" | "approval" | "approvedFileChangeIds" | "approvedExternalOperationIds"> => Object.fromEntries(
  Object.entries(plan).filter(([key]) => !["planHash", "approval", "approvedFileChangeIds", "approvedExternalOperationIds"].includes(key)),
...
const validateJournal = (journal: RecoveryJournal): string | undefined => {
  if (journal.schemaVersion !== 1 || journal.runId.length === 0 || journal.root.length === 0 || journal.planHash.length !== 64) return "Transaction journal has an invalid identity or schema";
  for (const entry of journal.entries) {
    if (entry.operationId.length === 0 || !isSafeRelativePath(String(entry.destination))) return `Transaction journal has an unsafe destination: ${entry.destination}`;
    if (entry.prior.existed && !isSafeRelativePath(String(entry.prior.backupPath))) return `Transaction journal has an unsafe backup path: ${entry.prior.backupPath}`;
  }
  return undefined;
};

const validatePlan = (plan: ApprovedPlan): string | undefined => {
  if (calculatePlanHash(unsignedPlan(plan)) !== plan.planHash || plan.approval.planHash !== plan.planHash) return "The approved plan hash is stale or inconsistent";
  ...
  if (new Set(plan.approvedFileChangeIds).size !== plan.approvedFileChangeIds.length || plan.approvedFileChangeIds.some((id) => !fileIds.has(id))) return "Approved file operation IDs are not a subset of the plan";
  if (new Set(plan.approvedExternalOperationIds).size !== plan.approvedExternalOperationIds.length || plan.approvedExternalOperationIds.some((id) => !externalIds.has(id))) return "Approved external operation IDs are not a subset of the plan";
  if (plan.approvedExternalOperationIds.some((id) => !plan.approval.networkOperations.includes(id))) return "An external operation is not explicitly network-approved";
  for (const change of plan.fileChanges) if (!isSafeRelativePath(String(change.destination))) return `Unsafe destination: ${change.destination}`;
  for (const operation of plan.externalOperations) {
    if (!isSafeRelativePath(String(operation.destination)) || operation.origin.length === 0 || operation.purpose.length === 0) return `Invalid external operation: ${operation.id}`;
...
export class PersistentTransactionEngine implements TransactionEngine {
  ...
  public async apply(plan: ApprovedPlan, signal: AbortSignal): Promise<TransactionResult> {
    ...
    let journal: RecoveryJournal = { schemaVersion: 1, runId: plan.runId, root: plan.root, planHash: plan.planHash, phase: "preparing", entries, manualReviewPaths: [] };
    try { await this.saveJournal(journal); } catch (cause) {
    ...
    try {
      ...
      if (!prepared.ok) {
        journal = { ...journal, entries: journal.entries.map((entry) => snapshots.find((snapshot) => snapshot.entry.operationId === entry.operationId)?.entry ?? entry) };
        return await this.failAndRollback(journal, receipts, prepared.error.message);
      }
      journal = { ...journal, phase: "prepared", entries: journal.entries.map((entry) => snapshots.find((snapshot) => snapshot.entry.operationId === entry.operationId)?.entry ?? entry) };
      ...
      await this.releaseLock(plan.runId);
      return { status: "committed", exitCode: 0, applied: receipts.map((receipt) => receipt.operationId), skipped: await this.computeSkipped(plan, receipts, snapshots), warnings: [], errors: [], manualReviewPaths: [] };
    } catch (cause) {
  ...
  public async recover(journal: RecoveryJournal): Promise<RecoveryResult> {
    ...
    if (invalid !== undefined) return { status: "incomplete", exitCode: 3, restored: [], manualReviewPaths: journal.manualReviewPaths, errors: [invalid] };
    if (terminal(journal.phase)) return { status: "restored", exitCode: 1, restored: [], manualReviewPaths: journal.manualReviewPaths, errors: [`Journal is already terminal: ${journal.phase}`] };
    ...
    try { await this.saveJournal(current); } catch (cause) { return { status: "incomplete", exitCode: 3, restored: [], manualReviewPaths: current.manualReviewPaths, errors: [textError(cause)] }; }
    ...
    if (result.manualReviewPaths.length > 0) {
      ...
      return { status: "incomplete", exitCode: 3, restored: result.restored, manualReviewPaths: result.manualReviewPaths, errors: result.errors };
    }
  ...
  private approvedEntries(plan: ApprovedPlan): JournalEntry[] {
    ...
    return changes.map((change) => ({
      ...
      prior: { existed: false },
      desiredDigest: change.afterDigest ?? (this.contents.has(change.id) ? digest(this.contents.get(change.id) as Uint8Array) : ("" as Sha256)),
      status: "pending",
  ...
  private async prepare(
    ...
    for (const entry of entries) {
      ...
      if (current.value.entry.prior.existed && change.afterDigest !== undefined && current.value.entry.prior.digest === change.afterDigest) continue;
      ...
      if (change.afterDigest !== undefined && result.value.desiredDigest !== undefined && change.afterDigest !== result.value.desiredDigest) return err(failure(`Prepared digest does not match the approved digest for ${entry.operationId}`));
    ...
    for (const operationDefinition of external) {
      ...
      if (operation === undefined) return err(failure(`No approved external operation implementation exists for ${operationDefinition.id}`));
  ...
  private async snapshot(plan: ApprovedPlan, change: FileChange, entry: JournalEntry, index: number): Promise<Result<Snapshot>> {
    ...
    if (!exists) {
      if (change.action === "modify" || change.beforeDigest !== undefined) return err(failure(`Modify precondition failed for ${change.destination}`, "VERIFY_FAILED"));
    ...
    if (change.afterDigest !== undefined && before === change.afterDigest) return ok({ entry: { ...entry, prior: { existed: true, digest: before, backupPath: backupPath(plan.runId, index) } } });
    if (change.action === "create") return err(failure(`Create precondition failed for ${change.destination}`, "VERIFY_FAILED"));
    if (change.beforeDigest !== undefined && before !== change.beforeDigest) return err(failure(`Concurrent change detected at ${change.destination}`, "VERIFY_FAILED"));
  ...
  private async verifyCommitted(prepared: PreparedOperation): Promise<Result<void>> {
    ...
    return actual === prepared.desiredDigest ? ok(undefined) : err(failure(`Committed digest mismatch at ${prepared.destination}`, "VERIFY_FAILED"));
  }
  ...
  private async persistManagedState(plan: ApprovedPlan): Promise<Result<ManagedState | undefined>> {
    ...
    const definitions = new Map((this.options.componentDefinitions ?? []).map((component) => [`${component.type}:${component.id}`, component]));
    const records = new Map<string, { component: Pick<ComponentDefinition, "id" | "type" | "source">; destinations: SafeProjectPath[]; contentDigest: Sha256 }>();
    for (const change of plan.fileChanges.filter((item) => plan.approvedFileChangeIds.includes(item.id) && item.action !== "preserve" && item.action !== "skip")) {
      const previous = loaded.value === undefined ? undefined : Object.entries(loaded.value.components).find(([key]) => key.endsWith(`:${change.componentId}`))?.[1];
      ...
      records.set(key, { component, destinations: [...(existing?.destinations ?? []), change.destination], contentDigest: change.afterDigest ?? existing?.contentDigest ?? ("0".repeat(64) as Sha256) });
    }
  ...
  private async failAndRollback(journal: RecoveryJournal, receipts: readonly CommitReceipt[], message: string, cancelled = false): Promise<TransactionResult> {
    const result = await this.rollback(journal, receipts);
    const errors = [message, ...result.errors];
    if (result.manualReviewPaths.length > 0) return { status: "incomplete", exitCode: 3, applied: receipts.map((receipt) => receipt.operationId), skipped: [], warnings: cancelled ? ["Execution cancelled"] : [], errors, manualReviewPaths: result.manualReviewPaths, journal: result.journal };
    await this.cleanupArtifacts(result.journal);
    await this.releaseLock(journal.runId);
    return { status: "rolled-back", exitCode: cancelled ? 0 : 1, applied: [], skipped: [], warnings: cancelled ? ["Execution cancelled"] : [], errors, manualReviewPaths: [], journal: result.journal };
  }

  private async rollback(journal: RecoveryJournal, receipts: readonly CommitReceipt[]): Promise<{ journal: RecoveryJournal; errors: string[]; manualReviewPaths: SafeProjectPath[] }> {
    let current: RecoveryJournal = { ...journal, phase: "rolling-back" };
    const errors: string[] = [];
    const paths: SafeProjectPath[] = [];
    try { await this.saveJournal(current); } catch (cause) { errors.push(textError(cause)); }
    for (const receipt of [...receipts].reverse()) {
      const operation = this.operations.get(receipt.operationId);
      if (operation !== undefined) {
        try {
          const undone = await operation.rollback(receipt);
          if (!undone.ok) { errors.push(undone.error.message); paths.push(receipt.destination); }
        } catch (cause) { errors.push(textError(cause)); paths.push(receipt.destination); }
      }
    }
    const restored = await this.restoreEntries(current);
    errors.push(...restored.errors);
    paths.push(...restored.manualReviewPaths);
    current = { ...current, entries: current.entries.map((entry) => restored.restored.includes(entry.operationId) ? { ...entry, status: "restored" as const } : entry), manualReviewPaths: [...new Set(paths)] };
    if (paths.length === 0) current = { ...current, phase: "rolled-back" };
    try { await this.saveJournal(current); } catch (cause) { errors.push(textError(cause)); }
    return { journal: current, errors, manualReviewPaths: [...new Set(paths)] };
  }

  private async restoreEntries(journal: RecoveryJournal): Promise<{ restored: string[]; errors: string[]; manualReviewPaths: SafeProjectPath[] }> {
    ...
    for (const entry of [...journal.entries].reverse()) {
      ...
      try {
        ...
        if (exists !== entry.prior.existed || (exists && digest(await this.fileSystem.read(entry.destination)) !== (entry.prior.existed ? entry.prior.digest : ""))) throw new Error(`Restored state mismatch for ${entry.destination}`);
        restored.push(entry.operationId);
      } catch (cause) { errors.push(textError(cause)); manualReviewPaths.push(entry.destination); }
    }
  ...
  private async cleanupArtifacts(journal: RecoveryJournal): Promise<void> {
    for (const entry of journal.entries) {
      ...
      try { if (await this.fileSystem.exists(entry.prior.backupPath)) await this.fileSystem.remove(entry.prior.backupPath); } catch { /* preserve the terminal journal */ }
    }
  }
  ...
  private async recoveryGate(root: CanonicalPath): Promise<string | undefined> {
    ...
    for await (const descriptor of entries) {
      ...
      } catch (cause) { return `Unable to inspect transaction journal ${path}: ${textError(cause)}`; }
    }
  ...
  private async acquireLock(root: CanonicalPath, runId: RunId): Promise<Result<void>> {
    ...
    if (await this.fileSystem.exists(ACTIVE_LOCK as SafeProjectPath)) {
      let owner = "unknown";
      try { owner = decoder.decode(await this.fileSystem.read(ACTIVE_LOCK as SafeProjectPath)); } catch { /* preserve the lock as evidence */ }
  ...
  private async releaseLock(runId: RunId): Promise<void> {
    ...
    try { if (await this.fileSystem.exists(ACTIVE_LOCK as SafeProjectPath)) await this.fileSystem.remove(ACTIVE_LOCK as SafeProjectPath); } catch { /* leave evidence for manual recovery */ }
  }
  ...
  private async writeDurable(path: SafeProjectPath, content: Uint8Array): Promise<Result<void>> {
    try {
      const result = this.fileSystem.writeAtomic !== undefined
      ...
      return ok(undefined);
    } catch (cause) { return err(failure(`Durable write failed for ${path}: ${textError(cause)}`, "WRITE_FAILED")); }
  }
  ...
  private async computeSkipped(plan: ApprovedPlan, receipts: readonly CommitReceipt[], snapshots: readonly Snapshot[]): Promise<string[]> {
    ...
    for (const change of plan.fileChanges) {
      if (!plan.approvedFileChangeIds.includes(change.id) || change.action === "preserve" || change.action === "skip") skipped.add(change.id);
      else if (!applied.has(change.id) && change.afterDigest !== undefined && await this.fileSystem.exists(change.destination) && digest(await this.fileSystem.read(change.destination)) === change.afterDigest) skipped.add(change.id);
    }
    for (const operation of plan.externalOperations) {
      if (!plan.approvedExternalOperationIds.includes(operation.id) || (!applied.has(operation.id) && await this.externalEquivalent(operation))) skipped.add(operation.id);
    }
    for (const snapshot of snapshots) if (!applied.has(snapshot.entry.operationId) && snapshot.entry.prior.existed && snapshot.entry.prior.digest === snapshot.entry.desiredDigest) skipped.add(snapshot.entry.operationId);
    return [...skipped].sort();
  }
  ...
  private cancelled(): TransactionResult {
    return { status: "rolled-back", exitCode: 0, applied: [], skipped: [], warnings: ["Execution cancelled before prepare"], errors: [], manualReviewPaths: [] };
  }
}

export class ContentFileOperation implements TransactionOperation {
  public constructor(private readonly fileSystem: AtomicFileSystemPort, private readonly change: FileChange, private readonly content: Uint8Array) {}
  ...
  public async verify(prepared: PreparedOperation): Promise<Result<void>> {
    return prepared.desiredDigest === digest(this.content) ? ok(undefined) : err(failure(`Prepared content digest mismatch for ${this.change.id}`, "VERIFY_FAILED"));
  }
  public async commit(prepared: PreparedOperation): Promise<Result<CommitReceipt>> {
    const written = this.fileSystem.writeAtomic !== undefined
  ...
  public async rollback(receipt: CommitReceipt): Promise<Result<void>> { void receipt; return ok(undefined); }
}

export const createTransactionEngine = (options: TransactionEngineOptions): PersistentTransactionEngine => new PersistentTransactionEngine(options);

...
const component = (
  ...
  source: ComponentDefinition["source"] = { kind: "builtin", origin: "test-suite" },
): ComponentDefinition => ({
...
const project = (fileSystem: FakeFileSystem, selected: readonly { definition: ComponentDefinition; incompatibleOverride?: boolean }[]) =>
  new ComponentInspectionProjection({ adapters: adapters(fileSystem), fileSystem }).project({
    root: root.value,
    stack,
...
describe("component inspection and projection", () => {
  it("projects all selected component types with complete provenance and destinations", async () => {
    ...
    } as ComponentDefinition & { mcp: unknown };
    const rule = {
      ...component("rule.testing", "agent-rule"),
      rule: { id: "testing", content: "Use deterministic tests." },
    } as ComponentDefinition & { rule: unknown };
    const command = {
      ...component("command.testing", "agent-command"),
  ...
  it("requires an explicit override before projecting an incompatible component", async () => {
    const fileSystem = new FakeFileSystem();
    const incompatible = {
      ...component("rule.incompatible", "agent-rule"),
      compatibility: { op: "stack" as const, category: "framework" as const, oneOf: ["react"] },
      rule: { id: "incompatible", content: "Only for React" },
    } as ComponentDefinition & { rule: unknown };

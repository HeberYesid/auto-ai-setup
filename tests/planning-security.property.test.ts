...
import { ApprovedNetworkGateway, DeterministicChangePlanner, ImmutableApprovalPolicy, SecretRedactor, asCanonicalPath, isSafeRelativePath, ok } from "../src/index.js";
import { LocalEventFactory } from "../src/infrastructure/observability/event-sink.js";
import type { ComponentDefinition, ExternalOperation, FileChange, NetworkGateway, PlanningInput, RunId, Sha256, ConfirmedStack } from "../src/domain/index.js";
...
const component: ComponentDefinition = { id: "property-component" as ComponentDefinition["id"], type: "agent-rule", name: "Property", description: "Property", compatibility: { op: "always" }, source: { kind: "builtin", origin: "test" } };
const stack: ConfirmedStack = { items: [], resolvedConflicts: [], digest };
const destinationArb = fc.array(fc.stringMatching(/^[a-z][a-z0-9-]{0,8}$/), { minLength: 1, maxLength: 3 }).map((parts) => `managed/${[...new Set(parts)].join("/")}.md`);
const file = (destination: string, action: FileChange["action"] = "create"): FileChange => ({ id: `id:${destination}`, componentId: component.id, destination: destination as FileChange["destination"], action, reason: "property", conflict: "none", preview: { kind: "text", content: "content", truncated: false } });
const planInput = (changes: readonly FileChange[]): PlanningInput => ({ runId: "property-run" as RunId, root: canonical.value, mode: "manual", stack, components: [component], fileChanges: changes, externalOperations: [], now: "2025-01-01T00:00:00.000Z" });
const operation = (id: string): ExternalOperation => ({ id: id as ExternalOperation["id"], componentId: component.id, kind: "skill-install", command: ["npx", "autoskills", "install", "property"], origin: "https://github.com/midudev/autoskills", destination: ".kiro/skills/property" as ExternalOperation["destination"], purpose: "property", usesNetwork: true, expectedFiles: [] });
...
describe("Task 6 correctness properties", () => {
  ...
  it("Property 11: deterministic and complete plan diff", async () => {
    await fc.assert(fc.asyncProperty(fc.uniqueArray(destinationArb, { minLength: 0, maxLength: 5 }), async (destinations) => {
  ...
  it("Property 12: approval decisions require the exact plan hash", async () => {
    await fc.assert(fc.asyncProperty(fc.boolean(), async (approved) => {
      const planner = new DeterministicChangePlanner();
      const built = await planner.build(planInput([file("managed/config.json", "create")]));
      expect(built.ok).toBe(true);
      if (!built.ok) return;
      const policy = new ImmutableApprovalPolicy();
      const decision = policy.evaluate(built.value, { planHash: built.value.planHash, globalApproved: approved, conflicts: {}, incompatibleComponents: [], networkOperations: [] });
  ...
  it("Property 13: hostile destinations are rejected lexically", () => {
    fc.assert(fc.property(fc.oneof(fc.constantFrom("../outside", "../../outside", "/tmp/outside", "C:/outside", "a\\..\\outside", "a/\0/b"), fc.string()), (candidate) => {
      if (candidate.includes("..") || candidate.includes("\\") || candidate.includes("\0") || candidate.startsWith("/") || /^[A-Za-z]:/.test(candidate)) expect(isSafeRelativePath(candidate)).toBe(false);
    }), { numRuns: 100 });
  });
  ...
  it("Property 14: redaction is non-leaking and idempotent", () => {
    fc.assert(fc.property(fc.stringMatching(/[A-Za-z0-9]{1,24}/), (secret) => {
  ...
  it("Property 23: events retain base fields and verbose context policy", () => {
    fc.assert(fc.property(fc.boolean(), (verbose) => {
      const factory = new LocalEventFactory({ clock: { now: () => "2025-01-01T00:00:00.000Z", monotonicMs: () => 0 }, verbose });
      const event = factory.create({ runId: "event-run" as RunId, level: "info", category: "plan", message: "plan", context: { evidence: "package.json" } });
  ...
  it("Property 25: foreign or stale network approvals never reach the delegate", async () => {
    await fc.assert(fc.asyncProperty(fc.stringMatching(/[a-z]{1,8}/), fc.boolean(), async (id, useExactId) => {
      let calls = 0;
